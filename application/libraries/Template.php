...
class Template
{
	...
	private $_scripts_footer = array(); // Damaso
	private $_nav; // Damaso
	...
	public function build($view, $data = array(), $return = FALSE)
	{
		...
		$template['scripts_footer']	= implode("\n\t\t", $this->_scripts_footer);
		$template['partials']	= array();
		$template['nav'] = $this->_nav;
	...
	public function add_js($script, $type = 'import', $section='footer') {
        switch ($type) {
            case 'import':
            	$base_url = $this->_ci->config->item('base_url');
            	preg_match('/^\/\//', $script, $matches);
            	$src = (empty($matches)) ? ($base_url . $script) : $script;
                $js = '<script src="'.htmlspecialchars(strip_tags($src)).'"></script>';
            break;
            case 'embed':
                $js = '<script>/*<![CDATA[*/' . $script . '/*]]>*/</script>';
            break;
        }

        if($section == 'footer'){
        	$this->_scripts_footer[] = $js;
        }else{
        	$this->append_metadata($js);
        }
    }

    public function add_css($style, $type = 'link', $rel='stylesheet', $media = 'all'){
    	switch ($type) {
            case 'link':
            	$base_url = $this->_ci->config->item('base_url');
            	preg_match('/^http/', $style, $matches);
            	$href = (empty($matches)) ? ($base_url . $style) : $style;
                $css = '<link rel="'. $rel .'" href="' . $href . '" media="' . $media .'">';
            break;
            case 'embed':
                $css = '<style type="text/css">' . $style . '</style>';
            break;
        }
        $this->append_metadata($css);
    }

    /**
     * Configura el menu principal para la plantilla
     * @param  string $nav Menu html
     * @return void
     */
